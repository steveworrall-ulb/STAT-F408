...
figure;
plot(Y(1,:), Y(2,:), 'b.', 'MarkerSize', 2, 'DisplayName', 'Direct');
hold on
plot(V(1,:), V(2,:), 'g.', 'MarkerSize', 2, 'DisplayName', 'MH');
plot(X(1,:), X(2,:), 'r.', 'MarkerSize', 2, 'DisplayName', 'Gibbs');
hold off
legend('Location', 'best')
xlabel('X_1'); ylabel('X_2');
title(['Sample clouds for d = ', num2str(dimension)]);